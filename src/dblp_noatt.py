...
S1 = (1-alpha) * np.linalg.inv((np.eye(A_apa.shape[0]) - alpha * A_apa.T))
S1 = np.asarray(S1)
S1 = S1 + S1.T
...
# A2_tilde = normalize_by_row_np(A)

S2 = (1-alpha) * np.linalg.inv((np.eye(A_apcpa.shape[0]) - alpha * A_apcpa.T))
S2 = np.asarray(S1)
S2 = S2 + S2.T
...
S3 = (1-alpha) * np.linalg.inv((np.eye(A_aptpa.shape[0]) - alpha * A_aptpa.T))
S3 = np.asarray(S3)
S3 = S3 + S3.T
